...
int main(void)
{
  ...
  //sprintf(myMsg,"Merhaba hello wrold \n");
  //print_uart(myMsg);
  /*
   * Normalde düz olarak bir kontrol flag i eklenmeden her iki task da aynı priorty ile başlatılırsa
   * bunlar aralarında sürekli birbirlerinin işini bitirmeden keserler
   *
   * Bunu engellemek için en basit şekilde bir flag ile bir task işleme başladığında flag değiştirilir ve diğer
   * task başladığında bu flag kontrol edildiğinde bu task başlatılmamış olunur  , yine ilk task işini bitirdikten
   * sonra flag eski haline getirilir ve diğer task için flag doğru olduğu için bu task çalışmaya başlar ve yine flag
   * bu task içinde de değiştirildiği için bu task bitene kadar diğer task çalışmamış olur
   */
