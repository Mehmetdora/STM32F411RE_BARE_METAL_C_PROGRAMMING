...
void ADC_init(int resolution){
	...
	// Tetikleme Kaynağı -> EXTSEL -> hangi timer ve modda tetikleneceği
	ADC1->CR2 &= ~(15UL << 24);
	ADC1->CR2 |= (6UL << 24);

	// Tetiklemenin hangi edge de yapılacağı
	ADC1->CR2 &= ~(3UL << 28);
	ADC1->CR2 |= (1UL << 28);  // rising edge




	// Continuous mod kapalı olmalı çünkü her tetiklemede bir kere conversion yapılacak
	ADC1->CR2 &= ~(1UL << 1);
	...
	// Start ADC, SWSTART -> yazılımsal olarak ADC nin başlatılmasıdır.
	// ADC1->CR2 |= (1UL << 30);
