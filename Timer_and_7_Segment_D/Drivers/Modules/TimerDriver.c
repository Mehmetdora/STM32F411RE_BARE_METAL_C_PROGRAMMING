...
void TimerDriver_init(void){
	...
	// Interrupt Enable
	TIM2->DIER |= (1UL << 0);	// 0. bit interrupt kontrol eder, 1:enable, 0:disable
...
void TIM2_IRQHandler(void){
	...
	if(TIM2->SR & (1UL << 0)){	// TIM2->SR ın 0. biti UIF flag i dir. Bu set olmuşsa interrupt olmuş demek olur
		...
		SegmentDisplayDriver_display(counter);	// değeri display ile göster

		GPIOA->ODR ^= (1UL << 5);
		//SegmentDisplayDriver_toggle();
