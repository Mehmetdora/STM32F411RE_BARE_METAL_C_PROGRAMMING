...
void ButtonDriverInt_init(){
	...
	// mode: input(00)
	GPIOC->MODER &= ~(3UL << (2*10));	// 13. pinini input yap
	//GPIOA->MODER &= ~(3UL << (2*4));	// 4. pinini input yap

	// pull up/down:
	GPIOC->PUPDR &= ~(3UL << 20);
	GPIOC->PUPDR |=  (1UL << 20);  // 13. pinini Pull-up yap
	...
	EXTI->FTSR |= EXTI_FTSR_TR10;	// falling edge için interrupt oluşsun
	EXTI->RTSR |= EXTI_RTSR_TR10;	// rising edge için interrupt oluşsun
...
void EXTI15_10_IRQHandler(void){	// bu özel ISR fonk isimleri startup_stm32.s dosyasında bulunur
	...
	if(EXTI->PR & EXTI_PR_PR10){
		...
		// Ayarlanan butona basılması ile tetiklenecek interrupt ile yaptırılmak istenen işlem buraya yazılır.
		...
		if(GPIOC->IDR & (1UL << 10) ){
			...
			if(TIM2->CCR1 < 1500 ){
				PWMDriver_set_CCR1(1600 + button_pressed_time);
			}else{
				PWMDriver_set_CCR1(1450 - button_pressed_time);
			}
