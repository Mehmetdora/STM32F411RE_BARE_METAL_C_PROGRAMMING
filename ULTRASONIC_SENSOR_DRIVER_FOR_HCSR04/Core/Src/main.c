...
int main(void)
{
  ...
  /*
   * Sensör başlatıldıkran 50 sn sonra doğru değerler ile ölçüm yapmaya başlıyor.
   *
   */
  HCSR_04_init();
  ...
  while (1)
  {
	  ...
	  {
	      measurement_done = 0;
	      distance = HCSR_04_GetDistance();

	      int2char(distance, distance_str + 6);
		  uart_send_string_IT(distance_str);
		  HAL_Delay(60);						// 60 ms aralıklarla ölçüm yapılıyor , nedeni sensör anlık ölçüm alamıyor, ölçümün tamamlanmasını bekleme süresi max
	      HCSR_04_start();
	  }
